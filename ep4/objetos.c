...
{
    if(pObj->categoria == FLOAT_STR || pObj->categoria == INT_STR || pObj->categoria == STR || pObj->categoria == ID)
...
mostreObjeto(CelObjeto *pObj, int tipoLista)
{
    if(tipoLista == ITENS){
        ...
        if((pObj->categoria) <= 18) /*se for um operador*/
        {
            printf("   prec=%d", pObj->valor.vInt);
        }
        else /* se nao for operador*/
        {
            printf("   %f", pObj->valor.vFloat);
            /* poderia ter usado mostraValor, mas essa funcao quebra linha*/
        }
        printf(" (%s)\n", nomeCategoria[pObj->categoria]);
    }
